...
def partial_corr(C, add_offset=False, return_pval=False, return_CI=False):
    C = np.asarray(C)
    ...
    P_corr_pval = np.zeros((p, p), dtype=np.float64)
    P_corr_CI = np.zeros((p, p, 2), dtype=np.float64)
    for i in range(p):
        P_corr[i, i] = 1
        for j in range(i + 1, p):
            ...
            if return_CI:
                res = stats.pearsonr(res_i, res_j)
                CI = res.confidence_interval(confidence_level=0.95)
                P_corr_CI[j, i, 0] = CI[0]
                P_corr_CI[j, i, 1] = CI[1]

    if add_offset:
        P_corr = P_corr[:-1, :-1]
        P_corr_pval = P_corr_pval[:-1, :-1]
        if return_CI:
            P_corr_CI = P_corr_CI[:-1, :-1,:]

    if return_pval:
        if return_CI:
            return P_corr, P_corr_pval, P_corr_CI
        else:
