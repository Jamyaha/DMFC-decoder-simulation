...
% 2023/7/3: get PVAF for train (to make sure it's positive)

% 2024/6/13: egocentric ball position (ball XY - eye XY) %%%%%
...
%% preprocessing
% generate egocentric ball positions
% [vis,occ]=convert_to_egocentric(vis,occ); %%%%%
[vis,occ]=convert_to_egocentric_ball_only(vis,occ); %%%%%
...
PVAF_train_vis=nan(size(Dev_model_vis));
PVAF_train_occ=nan(size(Dev_model_occ));
...
for iNeuron=1:size(r_vis,1)
    ...
    for iModel=1:nModel
        ...
        [Dev_model_vis(iNeuron,iModel),LL_model_vis(iNeuron,iModel),...
            PVAF_model_vis(iNeuron,iModel),PVAF_train_vis(iNeuron,iModel)]=run_glm_fit(tmpY,tmpX,tmpC);
        % aicbic
        nObs=size(nr_vis,2);
...
for iNeuron=1:size(r_occ,1)
    ...
    for iModel=1:nModel
        ...
        [Dev_model_occ(iNeuron,iModel),LL_model_occ(iNeuron,iModel),...
            PVAF_model_occ(iNeuron,iModel),PVAF_train_occ(iNeuron,iModel)]=run_glm_fit(tmpY,tmpX,tmpC);
        % aicbic
        nObs=size(nr_occ,2);
        nParams=sum(id_mat(iModel,:))+1; % constant
...
% save run_glm.mat;
% save run_glm_egocentric_ball_paddle.mat;
save run_glm_egocentric_ball.mat;

%% subfunctions
%%
function [vis,occ]=convert_to_egocentric(vis,occ) %%%%%
%%
% convert to allocentric ball positions to egocentric
vis.ball_pos_x = vis.ball_pos_x - vis.eye_h;
vis.ball_pos_y = vis.ball_pos_y - vis.eye_v;
vis.paddle_pos_y = vis.paddle_pos_y - vis.eye_v;
vis.ball_pos_x_TRUE = vis.ball_pos_x_TRUE - vis.eye_h;
vis.ball_pos_y_TRUE = vis.ball_pos_y_TRUE - vis.eye_v;

occ.ball_pos_x = occ.ball_pos_x - occ.eye_h;
occ.ball_pos_y = occ.ball_pos_y - occ.eye_v;
occ.paddle_pos_y = occ.paddle_pos_y - occ.eye_v;
occ.ball_pos_x_TRUE = occ.ball_pos_x_TRUE - occ.eye_h;
occ.ball_pos_y_TRUE = occ.ball_pos_y_TRUE - occ.eye_v;

function [vis,occ]=convert_to_egocentric_ball_only(vis,occ) %%%%%
%%
% convert to allocentric ball positions to egocentric
vis.ball_pos_x = vis.ball_pos_x - vis.eye_h;
vis.ball_pos_y = vis.ball_pos_y - vis.eye_v;
...
function [DEV,LL,PVAF,PVAF0]=run_glm_fit(Y,X,varargin)
%%
...
for i=1:c.NumTestSets
    ...
    % compute PVAF for train
    nHat0=glmval(B,X(training(c,i),:),'identity'); % ,'log'); %
    SS_res0=var(Y(training(c,i))-nHat0);
    SS_tot0=var(Y(training(c,i)));
    PVAF0=1-SS_res0./SS_tot0;
end
