...
PATH_TO_FIXED_POINT_FINDER = '/Users/hansem/Dropbox (MIT)/MPong/phys/code/fixed-point-finder/' # '/om/user/rishir/lib/fixed-point-finder/'
sys.path.insert(0, PATH_TO_FIXED_POINT_FINDER)
PATH_TO_FIXED_POINT_FINDER = '/Volumes/NILAB/MPong/rishir/lib/PongRnn/dat/' # '/Volumes/Analogy_G_s/MPong/om_rishir/lib/PongRnn/dat/' # '/om/user/rishir/lib/PongRnn/'
sys.path.insert(0, PATH_TO_FIXED_POINT_FINDER)

RNN_RES = '/Volumes/NILAB/MPong/rishir/lib/PongRnn/dat/rnn_res' # CNIR10 NAS
# RNN_RES = '/Volumes/Analogy_G_s/MPong/om_rishir/lib/PongRnn/dat/rnn_res' # SanDisk

PONG_BASIC_RF = '/Volumes/NILAB/MPong/rishir/data/pong_basic/RF/occluded_pong_bounce1_pad8_4speed/valid' # CNIR10 NAS
...
from phys.code.PongDatasets.PongDataset import map_batch_output
from sklearn.decomposition import PCA
from phys.code.rnn_analysis import utils
from phys.code.rnn_analysis import rnn_analysis_utils as RAU
...
class PongRNNSummarizer(object):
    ...
    def __init__(self, **kwargs):
        ...
        try:
            self.dat = pk.load(open(self.filename, 'rb')) # res[fk] = pk.load(open(fn, 'rb'))
        except:
            self.filename= '%s/%s/%s' % (RNN_RES,self.filename.split('/')[-2], self.filename.split('/')[-1])
            self.dat = pk.load(open(self.filename, 'rb'))  # res[fk] = pk.load(open(fn, 'rb'))
    ...
    def get_results_summary_base(self):
        ...
        for batch_idx, batch_fn in enumerate(batch_fns):
            try:
                f = h5py.File(batch_fn, 'r')
            except:
                batch_fn='%s/%s' % (PONG_BASIC_RF,batch_fn.split('/')[-1])
                f = h5py.File(batch_fn, 'r')
    ...
    def get_model_characterization(self, characteristics_oi=None, niter=2):
        ...
        dist_fn = '/Users/hansem/Dropbox (MIT)/MPong/lib/MentalPong/pong_basic/RF/occluded_pong_bounce1_pad8_4speed/train_to_valid_dist.pkl'
